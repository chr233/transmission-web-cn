...
TorrentRendererHelper.formatETA = function(t) {
    ...
    return "剩余: " + Transmission.fmt.timeInterval(eta);
};
    ...
    getPeerDetails: function(t) {
        ...
        if (t.isDownloading()) {
            ...
            if (webseed_count && peer_count) {
                ...
                return ['下载/活跃/BT: ',
                    peer_count + '/' + t.getPeersSendingToUs() + '/' + webseed_count,
                    '–',
                ...
                return ['下载/活跃: ',
                    peer_count + '/' + t.getPeersSendingToUs(),
                    '–',
                    TorrentRendererHelper.formatDL(t),
                    TorrentRendererHelper.formatUL(t)
                ].join(' ');
        ...
        if (t.isSeeding()) {
            return ['上传/活跃: ', t.getPeersConnected() + '/' + t.getPeersGettingFromUs(), '-', TorrentRendererHelper.formatUL(t)].join(' ');
        };

        if (t.isChecking()) {
            return ['校验中 (', Transmission.fmt.percentString(100.0 * t.getRecheckProgress()), '%)'].join('');
        }
    ...
    getProgressDetails: function(controller, t) {
        ...
        if (is_done) {
            if (totalSize === sizeWhenDone) {
                // seed: '698.05 MiB'
                c = ['做种中: ', Transmission.fmt.size(totalSize)];
            } else { // partial seed: '127.21 MiB of 698.05 MiB (18.2%)'
                c = ['做种中: ', Transmission.fmt.size(sizeWhenDone), ' / ', Transmission.fmt.size(t.getTotalSize()), ' (', t.getPercentDoneStr(), '%)'];
            };
        ...
        //         c.push('剩余: ∞');
        //     } else {
        //         c.push(Transmission.fmt.timeInterval('剩余: ', t.getETA()));
        //     };
        // };
...
TorrentRendererCompact.prototype = {
    ...
    getPeerDetails: function(t) {
        ...
        if (t.isSeeding()) {
            return ['分享率: ', Transmission.fmt.ratioString(t.getUploadRatio()), ', ', TorrentRendererHelper.formatUL(t)].join('');
        };
        return t.getStateString();
    },

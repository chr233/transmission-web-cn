/**
 * Copyright Ã‚Â© Jordan Lee, Dave Perrett, Malcolm Jarvis and Bruno Bierbaumer
 *
        ...
        updatePeersPage = function() {
            ...
            for (k = 0; tor = torrents[k]; ++k) {
                ...
                html.push('<table class="peer_list">',
                    ...
                    '<th class="percentCol">进度</th>',
                    '<th class="statusCol">状态</th>',
                    '<th class="addressCol">IP</th>',
                    '<th class="clientCol">客户端</th>',
                    '</tr>');
                for (i = 0; peer = peers[i]; ++i) {
                    parity = (i % 2) ? 'odd' : 'even';
                    html.push('<tr class="inspector_peer_entry ', parity, '">',
                        '<td>', (peer.isEncrypted ? '<div class="encrypted-peer-cell" title="连接已加密">' : '<div class="unencrypted-peer-cell">'), '</div>', '</td>',
            ...
            for (i = 0; tor = torrents[i]; ++i) {
                ...
                for (j = 0; tracker = trackers[j]; ++j) {
                    ...
                    html.push('<li class="inspector_tracker_entry ', parity, '"><div class="tracker_host" title="', sanitizeText(tracker.announce), '">',
                        ...
                        '<tr><th>做种:</th><td>', (tracker.seederCount > -1 ? tracker.seederCount : na), '</td></tr>',
                        '<tr><th>活跃:</th><td>', (tracker.leecherCount > -1 ? tracker.leecherCount : na), '</td></tr>',
                        '<tr><th>下载:</th><td>', (tracker.downloadCount > -1 ? tracker.downloadCount : na), '</td></tr>',
                        '</table></li>');
                }
